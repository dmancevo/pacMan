...
from ai_util import computeVectorStateRepresentation
...
class baseAgent(CaptureAgent):
  ...
  def init(self, gameState):
    ...
    for x in xrange(redFood.width):
      for y in xrange(redFood.height):
        if redFood:
          self.foodPerTeam += 1
  ...
  def chooseAction(self, gameState):
    ...
    '''
    self.updateFoodMap(gameState)
    beliefState = Filter.getBeliefStateBool()
    state = computeVectorStateRepresentation(self.index, gameState,
                                              self.distancer,
                                              baseAgent.moveMap,
                                              baseAgent.foodMap,
                                              baseAgent.defendFoodMap,
                                              beliefState,
                                              self.foodPerTeam)
    ...
    #MY TEST
    successor = self.getSuccessor(gameState, bestDir)
    nextEnemy = 0 if self.index == 3 else self.index + 1
    print successor.getAgentState(nextEnemy).numCarrying
    print successor.getAgentState(nextEnemy).isPacman
    #nextEnemyActions = successor.getLegalActions(nextEnemy)
    #print nextEnemyActions
    #successor2 = successor.getSuccessor(gameState, action)
